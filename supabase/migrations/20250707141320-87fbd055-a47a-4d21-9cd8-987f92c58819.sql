...
-- 3. Create a view for customer analytics
...
WITH customer_stats AS (
  SELECT 
    ...
    COUNT(b.id) as total_bookings,
    SUM(b.total_amount) FILTER (WHERE b.status IN ('completed', 'active')) as total_spent,
    AVG(b.total_amount) FILTER (WHERE b.status IN ('completed', 'active')) as avg_booking_value,
...
SELECT 
  *,
  CASE 
    ...
    WHEN recent_bookings > 0 THEN 'Active'
    WHEN last_booking_date < NOW() - INTERVAL '90 days' THEN 'At Risk'
    ELSE 'Regular'
...
-- 4. Create a view for booking patterns and peak hours
...
-- 5. Create a function to get real-time analytics dashboard data
...
-- 6. Create a function to get vehicle category performance
